...
test.describe('Динамический контент с условиями', () => {
  ...
  test('Фильтрация динамических элементов', async ({ page }) => {
    ...
    const dynamicButton = page.locator('.disabled').filter({ hasText: 'Недоступно' }); // TODO(student): замените на корректный локатор
    await expect(dynamicButton).toBeVisible({ timeout: 2000 });

    // 2. Найти динамический товар, который:
    //    - Цена меньше 10 000 ₽
    //    - Не является рекомендуемым (featured)
    const cheapProduct = page.locator(`div[id='dynamic-content'] span[class='price-value']`); // TODO(student): замените на корректный локатор
  ...
  test('Комбинации с :has и :not', async ({ page }) => {
    ...
    const availableProducts = page
      ...
      .filter({ hasText: 'В корзину' }); // TODO(student): замените на корректный локатор
