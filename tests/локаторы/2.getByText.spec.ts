...
test.describe('Поиск по частичному совпадению', () => {
  ...
  test('Найти по частичному совпадению', async ({ page }) => {
    const partialText = page.getByText('важную информацию', { exact: false }); // TODO(student): замените на корректный локатор
  ...
  test('Найти элемент списка по части текста', async ({ page }) => {
    const listItem = page.getByRole('listitem').getByText('Специальный'); // TODO(student): замените на корректный локатор
